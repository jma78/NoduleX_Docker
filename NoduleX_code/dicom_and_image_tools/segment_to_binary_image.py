...
def is_polygon(candidate):
    has_coords = ('has_segmentation' in candidate and candidate['has_segmentation'] == True)
...
def build_choices_list(polygons, centroids=[]):
    ...
    for center in centroids:
        ...
        for pidx, poly in enumerate(polygons):
            ...
            if poly['sliceNo'] == rz and point_is_inside_coords((rx, ry), poly['coords']):
                is_inside = True
                eprint("{0} is inside poly {1}".format([round(p) for p in (center['x'], center['y'], center['sliceNo'])], pidx+1))
                break
        if not is_inside:
...
def choose_candidate(polygons, centroids=[], auto_choose=None):
    ...
    if strategy_name == 'most-malignant':
        ...
        print(" - filtering by malignancy filter (threshold: {}, comparision: {})".format(threshold, comparison))
        return auto_choose_candidate_filter_by_malignancy(polygons, centroids, threshold, comparison)
    elif strategy_name == 'malignancy-range':
        low = None; high = None; sort_order = None; seed = None
        ...
        print(" - filtering by Malignancy range {} to {}, randomize is {} seed is {}.".format(low, high, randomize, seed))
        return auto_choose_candidate_filter_by_malignancy_range(polygons, centroids, low, high, direction, randomize, seed)
        ...
        if strategy_args is not None:
            if strategy_args[0].lower() == "false" or int(strategy_args[0]) == 0:
                is_nodule_choice = 0
        return auto_choose_candidates_by_is_nodule(polygons, centroids, is_nodule_choice)
...
def auto_choose_candidate_filter_by_malignancy(polygons, centroids, threshold=0, comparison='>'):
    combined_list = auto_choose_candidate_all(polygons, centroids)
    combined_list = sort_candidates_by_area(combined_list)
    combined_list = sort_candidates_by_malignancy(combined_list, direction)
    return filter_candidates_by_malignancy(combined_list, threshold, comparison)

def auto_choose_candidate_filter_by_malignancy_range(polygons, centroids, low=1, high=5, direction=1, randomize=False, seed=None):
    print("Choosing by malignancy range {}-{} (random: {}, seed {})".format(low,high,randomize,seed))
    if low > high:
        low, high = (high, low)
    if low > 0:
        centroids = [c for c in centroids if 'malignancy' in c and c['malignancy'] > 0]  # Only use centroids if they are marked for malignancy (original ones aren't)
    combined_list = auto_choose_candidate_all(polygons, centroids)
    combined_list = sort_candidates_by_area(combined_list)
    combined_list = sort_candidates_by_malignancy(combined_list, direction)
...
def filter_candidates_by_malignancy(candidates, threshold, comparison='>'):
    ...
    '''
    filter_func = lambda x: x['malignancy'] >  threshold if comparison == '>'  else \
                  lambda x: x['malignancy'] <  threshold if comparison == '<'  else \
                  lambda x: x['malignancy'] >= threshold if comparison == '>=' else \
                  lambda x: x['malignancy'] <= threshold if comparison == '<=' else \
...
def filter_candidates_by_area(candidates, threshold, comparison=1):
    ...
    '''
    candidates  = set_area_for_all_candidates(candidates)
    filter_func = lambda x: x['area'] >  threshold if comparison == '>'  else \
                  lambda x: x['area'] <  threshold if comparison == '<'  else \
                  lambda x: x['area'] >= threshold if comparison == '>=' else \
                  lambda x: x['area'] <= threshold if comparison == '<=' else \
...
def filter_candidates_by_volume(candidates, threshold, comparison=1):
    ...
    '''
    candidates  = set_area_for_all_candidates(candidates)
    filter_func = lambda x: x['volume'] >  threshold if comparison == '>'  else \
                  lambda x: x['volume'] <  threshold if comparison == '<'  else \
                  lambda x: x['volume'] >= threshold if comparison == '>=' else \
                  lambda x: x['volume'] <= threshold if comparison == '<=' else \
...
def filter_groups(candidate_groups, auto_choose=None):
    ...
    if strategy_name == 'most-malignant':
        ...
        threshold, comparison = strategy_args
        threshold = int(threshold)
        print(" - filtering groups by malignancy filter (threshold: {}, comparision: {})".format(threshold, comparison))
        ...
        direction = -1 if sort_order == 'ascending' else 1
        randomize = sort_order == 'random'
        print(" - filtering groups by Malignancy range {} to {}, randomize is {} seed is {}.".format(low, high, randomize, seed))
        return filter_groups_filter_by_malignancy_range(candidate_groups, low, high, direction, randomize, seed)
    elif strategy_name == 'largest':
...
def filter_groups_filter_by_malignancy_range(candidate_groups, low=1, high=5, direction=1, randomize=False, seed=None):
    print("Choosing by malignancy range {}-{} (random: {}, seed {})".format(low,high,randomize,seed))
    if low > high:
        low, high = (high, low)
    if low > 0:
        candidate_groups = [c for c in candidate_groups if 'malignancy' in c and c['malignancy'] > 0]  # Only use centroids if they are marked for malignancy (original ones aren't)
    ...
    while not done:
        if refresh:
            ...
            for idx, choice in enumerate(choices):
                value       = choice['value']
                description = "mal: {0}, x: {1}, y: {2}, slice: {3}, area: {5}, id: {4}".format(value['malignancy'], round(value['x']), round(value['y']), value['sliceNo'], value['noduleID'], value['area'])
                print("[{0}]: {1} {2}".format(idx+1, choice['type'], description))
            refresh = False
        v = raw_input("Type # to choose, -# to remove, R to reload list, or X to exit: ")
        if v.lower() == 'r':
            refresh = True
        elif v.lower() == 'x':
            sys.exit(0)
...
def set_area_for_all_candidates(candidates):
    for idx, polygon in enumerate(candidates):
        if not 'area' in polygon:
            polygon['area'] = area_for_polygon(polygon['coords']) if 'coords' in polygon else 0
...
def centroid_to_polygon(centroid, max_area=None):
    ...
    candidates = find_candidate_nodules(image, seed_point=(x,y), seed_area=True, max_diameter=max_diameter)
...
def get_3d_boundaries(chosen, polygons, centroids=[], area_tolerance=1):
    ...
    while j >= 0 and (last_slice - by_slice[j][0]) <= 1: # moving to smaller-numbered slices
        ...
        eprint(chosen['value']['y'], chosen['value']['x'], center, (abs(area - last_area) / float(last_area)) <= area_tolerance, (abs(area - last_area) / float(last_area)))
        if point_is_inside_coords(center, last_boundary) and (abs(area - last_area) / float(last_area)) <= area_tolerance:
            eprint("appending")
    ...
    last_boundary = chosen_boundary_coords
    last_area     = chosen['value']['area'] + area_epsilon
    while j < len(by_slice) and (by_slice[j][0] - last_slice) <= 1: # moving to larger-numbered slices
        ...
        if point_is_inside_coords(center, last_boundary) and (abs(area - last_area) / float(last_area)) <= area_tolerance:
            ...
            last_area     = candidate['value']['area'] + area_epsilon
        j += 1
    adjacent_slices.sort(key=lambda x: x['value']['sliceNo']+0.000001*x['value']['malignancy'])
    for slice in adjacent_slices:
        eprint("Selected slice {0} x: {1} y: {2}".format(slice['value']['sliceNo'], slice['value']['x'], slice['value']['y']))
...
def produce_3d_binary_image(dicomdir, outputdir, slice_boundaries, shape=(512,512)):
    ...
    if os.path.abspath(outputdir) == os.path.abspath(dicomdir):
        raise IOError('Cannot output segmented images to same directory as original images.')
    ...
    for idx, slice in enumerate(slice_boundaries):
        ...
        center_x = slice['value']['x'] if center_x == None else center_x + slice['value']['x']
        center_y = slice['value']['y'] if center_y == None else center_y + slice['value']['y']
        center_z = slice['value']['z'] if center_z == None else center_z + slice['value']['z']
        center_slice = slice['value']['sliceNo'] if center_slice == None else center_slice + slice['value']['sliceNo']
        if fname in included_slices_by_name:
            included_slices_by_name[fname].append(slice)
        else:
            included_slices_by_name[fname] = [slice]
    center_x, center_y, center_z, center_slice = [int(round(float(v) / len(slice_boundaries))) for v in [center_x, center_y, center_z, center_slice]]
    ...
    with open(info_filename, 'w') as info_fp:
        ...
        info_fp.write('{0}, {1}, {2}, {3}, {4}\n'.format(center_x, center_y, center_z, center_slice, len(dicomfiles)))
...
def get_vote_threshold(consensus='vote'):
    ...
    if consensus[0:4] == 'vote' and len(consensus) > 4:
        ...
        if vote_threshold == 0:
            vote_threshold += 1e-10; # epsilon prevents perfect zero to require at least one vote
...
def write_3d_binary_image(dicomdir, outputdir, segmentation_group, shape=(512,512), consensus='vote'):
    """
    Valid consensus strategies are: 
    {'vote' (default), 'vote:PCT', 'union', 'intersect'}
        'vote'      : any pixel within 50% or more of boundaries is included
    ...
    for idx, slice in enumerate(segmentation_group['members']):
        ...
        if 'fileName' not in slice:
            slice['fileName'] = os.path.join(dicomdir, dicomfiles_by_slice[slice['sliceNo']])
            print("Filling in file name {} for slice {}.".format(slice['fileName'], slice['sliceNo']))
    ...
    for file in dicomfiles:
        ...
        if filepath in included_slices_by_name:
            ...
            for s_idx, slice in enumerate(included_slices_by_name[filepath]):
                print('Drawing pixels for object {} on slice {}'.format(s_idx+1, slice['sliceNo']))
                pixels = np.array(pixels) + draw_polygon_from_coords(pixels, slice['coords'])
            ...
            if n_objects_on_slice > pixels.max():
                eprint("WARNING: Multiple segmentations on same slice detected; slice {}".format(included_slices_by_name[filepath][0]['sliceNo']))
    ...
    info_filename = os.path.join(outputdir, 'segment_info.txt')
    with open(info_filename, 'w') as info_fp:
        info_fp.write('Segmentation Centroid: malignancy, x, y, z, sliceNo, totalSlices\n')
        info_fp.write('{0}, {1}, {2}, {3}, {4}, {5}\n'.format(center_malignancy, center_x, center_y, center_z, center_slice, len(dicomfiles)))
            ...
            if line.strip() != '':
                ...
                if len(parts) == 7:
                    ...
                    centroids.append({
                            ...
                            'noduleID': make_unique_id_string(int(round(float(x))), int(round(float(y))), int(slice_no), is_nodule, inclusion, malignancy, patient)
                        })
                else:
                    ...
                    centroids.append({
                            ...
                            'inclusion' : int(inclusion),
                            'malignancy': int(malignancy),
                            'noduleID': make_unique_id_string(int(round(float(x))), int(round(float(y))), int(slice_no), is_nodule, inclusion, malignancy)
                        })
    return centroids
...
def get_binary_segmentation_2d(candidate, img_shape=(512,512), min_radius=3):
    ...
    slice_bin    = draw_polygon_from_coords(slice_bin, c['coords']) \
                        if 'coords' in c \
                        else draw_circle(slice_bin, (int(round(c['x'])), int(round(c['y']))), min_radius)
    return slice_bin
...
def consolidate_candidates(candidates, consensus='vote', min_radius=3, img_shape=(512,512), keep_bin_image=False):
    """
    ...
    candidates = consolidate_candidates_2d(candidates, consensus, min_radius, img_shape, keep_bin_image=True)
    candidates = consolidate_candidates_3d(candidates, consensus, min_radius, img_shape, keep_bin_image=keep_bin_image)
    return candidates

def consolidate_candidates_2d(candidates, consensus='vote', min_radius=10, img_shape=(512,512), keep_bin_image=False):
    """
    Consolidate any candidates (from 2d slices) that are part of the same 
    "nodule-like-object" into a single list representing a 3-d candidate.
    ...
    for s in candidates_by_slice:
        ...
        slice_candidates = sorted(slice_candidates, key=lambda c: c['area'] if 'area' in c else 0)
        ...
        while(len(unmatched) > 0):
            ...
            for idx, m in enumerate(merged):
                ...
                if ((merged_bin != 0) & (slice_bin != 0)).any():  # If any pixel overlaps...
                    ...
                    print('Check {0} {1} inside? {2}'.format(center_c, center_r, merged_bin[center_r, center_c]))
                    if merged_bin[center_r, center_c] != 0:
                        ...
                        if is_polygon(c) and (not merged[idx]['has_segmentation']):
                            ...
                            merged[idx]['seg_map'][:]       = 0  # This will be the first "real" segmentation info.
                        ...
                        if is_polygon(c) or (not merged[idx]['has_segmentation']):
                            ...
                            print("Adding new segmentation to existing... seg idx {} count now {}".format(idx, merged[idx]['merged_count']))
                                        
            if not found_match:
                # If we couldn't merge the new candidate, it becomes a new 'merged' nodule on its own:
                c['seg_map']          = slice_bin # Note: new candidates created from points only will contain the default radius.
                c['merged_count']     = 1
        ...
        for idx, m in enumerate(merged):
            ...
            print("Need {} votes to keep voxel. (Merged {} candidates.)".format(threshold, m['merged_count']))
            ...
            if np.isnan(c_of_m).any(): # TODO: REMOVE
                print("NaN detected. Slice {}\n{}".format(m['sliceNo'],slice_bin[slice_bin != 0]))
                import scipy.misc
            ...
            if m['has_segmentation']:
                ...
                for o_i in range(num_objects):
                    print("Creating polygon for object {} of {}...".format(o_i+1, num_objects))
        ...
        if not keep_bin_image:
            for idx, m in enumerate(merged):
                for key in ['seg_map', 'merged_count', 'has_segmentation', 'seg_map_ul_x', 'seg_map_ul_y']:
                    try:
                        del merged[idx][key]
                    except:
                        pass
...
def consolidate_candidates_3d(candidates, consensus='vote', min_radius=3, img_shape=(512,512), keep_bin_image=False):
    """
    Consolidate any candidates (from 2d slices) that are part of the same 
    "nodule-like-object" into a single list representing a 3-d candidate.
    ...
    for candidate in candidates:
        if not 'seg_map' in candidate:
            candidate['seg_map'] = get_binary_segmentation_2d(candidate, img_shape, min_radius)
    ...
    for group_idx, group in enumerate(merged):
        group_malignancy = group_inclusion = group_is_nodule = group_has_segmentation = 0
        ...
        for member_idx, member in enumerate(group):
            ...
            group_has_segmentation = max(group_has_segmentation, int(member['has_segmentation']))
...
def verify_candidate_segmentations(candidates, dicom_dir, tmp_dir=None, candidate_limit=None, volume_max_threshold=100000, volume_min_threshold=4):
    import auto_segment_around_seed
    from tempfile import mkdtemp
    from shutil import rmtree
    tmp_dir         = mkdtemp(dir=tmp_dir if (tmp_dir is not None) and (os.path.isdir(tmp_dir)) else None )
    nii_file        = None
    nii_img         = None
    n_slices        = 0
    height          = 0
    ...
    for candidate in candidates:
        if is_polygon(candidate):
            ...
            seed     = (candidate['x']+1, height - candidate['y'] - 1, n_slices - candidate['sliceNo'])
            seed     = tuple([int(round(v)) for v in list(seed)])
            print("candidate at {} {} {} : seed: {}".format(candidate['x'], candidate['y'], candidate['sliceNo'], seed))
            run_info      = None
            consensus_img = None
            try:
            ...
            if run_info is not None and consensus_img is not None and type(consensus_img) != type(np.array([])):
                ...
                try:
                    ...
                    print("Exception when trying to get array from segmentation image: {}".format(exc))
                    consensus_img = None
                    candidate     = None
                    continue
                ...
                if pix_volume > 0 and pix_volume <= volume_max_threshold:
                    ...
                    seg_group['members'] = []
                    consensus_img = consensus_img[::-1,::-1]  # put the image back in our usual orientation
                    ...
                    for slice_idx in range(img_shape[0]):
                        if consensus_img[slice_idx].sum() > 0:
                            ...
                            img_labels, num_objects = measurements.label(consensus_img[slice_idx])
                            for o_i in range(num_objects):
                                print("Creating polygon for object {} of {}...".format(o_i+1, num_objects))
                                ...
                                isolated_object[isolated_object != 0]    = 1
                                boundary, coords   = get_polygon_from_binary_img(isolated_object)
                                slice_polygon_dict = make_polygon_dict_given_boundary(boundary, coords)
                                ...
                                slice_polygon_dict['x'], slice_polygon_dict['y'] = slice_polygon_dict['y'], slice_polygon_dict['x']
                                #[END]
                                slice_polygon_dict['noduleID']   = nodule_unique_id(slice_polygon_dict)
                    ...
                    print("Generated segmentation outside allowed volume range: {} vs {}".format(pix_volume, volume_max_threshold))
                consensus_img = None
            else:
                candidate     = None
                consensus_img = None
...
def filter_by_seed_points(candidates, seed_points, consensus='vote', image_shape=(512,512)):
    """
    ...
    for candidate_group in candidates:
        ...
        for slc in group_by_slice:
            ...
            object_labels, n_objects_on_slice = measurements.label(np.array(pixels, dtype=bool))
            ...
            for seed in seed_points:
                x, y, sliceNo = int(round(seed['x'])), int(round(seed['y'])), int(round(seed['sliceNo']))
                if sliceNo == slc:
                    if pixels[y][x] > 0:
                        keep_candidate = True
...
def get_args():
    ...
    ap.add_argument("output_dir", metavar='output-dir' , help = "Name of destination directory.")
    ap.add_argument("--tmp", dest = 'tmp_dir', required = False, default = '/tmp', help = "Directory for storing temporary files. (default=/tmp)")
    ap.add_argument("--candidates", dest = 'candidate_file', required = False, help = "File containing seed coordinates; these will override any XML candidates present in the DICOM directory.")
    ap.add_argument("--seed", dest='seed_coords', metavar=('x','y','z'), nargs=3, required = False, help = "Coordinates ((x y z) where z is slice #) of a custom seed point; will override any XML candidates present in the DICOM directory.")
    ap.add_argument("--consensus", dest='consensus_strategy', required=False, default='vote',
    ...
    ap.add_argument("--limit", dest='candidate_limit', metavar='N', required=False, help="If more than one candidate passes filters, limit to first N candidates. (default=all)")
    ap.add_argument("--segmented-only", dest='segmented_only', action='store_true', 
    ...
    ap.add_argument("--output-prefix", dest='nodule_id_prefix', metavar='PREFIX', required=False, help="Prepend PREFIX to all output directory names.")
    ap.add_argument("--auto", dest='auto_choose', metavar='STRATEGY', required=False, 
...
def run_standalone():
    ...
    candidate_limit         = int(args['candidate_limit']) if args['candidate_limit'] is not None else None
    ...
    selected_segmented_only = (candidate_file is not None or seed_coords is not None) and segmented_only == True
    ...
    if seed_coords is not None:
        ...
        centroids.append({
                ...
                'inclusion' : -1,
                'malignancy': -1,
                'noduleID':  make_unique_id_string(x, y, slice_no, -1, -1, -1, nodule_id_prefix)
            })
    
    if selected_segmented_only:
        selected_seed_coords = [{'x': c['x'], 'y': c['y'], 'z': c['z'], 'sliceNo': c['sliceNo']} for c in centroids]
        centroids = []
    ...
    while(retry and len(chosen_candidates) > 0):
        ...
        verified_candidates = verify_candidate_segmentations(filtered_candidates, dicom_dir=dicom_dir, tmp_dir=tmp_dir, candidate_limit=candidate_limit)
    ...
    if selected_segmented_only:
        chosen_candidates = filter_by_seed_points(chosen_candidates, selected_seed_coords, consensus=consensus_strategy)
    ...
    for chosen in chosen_candidates:
        ...
        output_path       = os.path.join(output_dir, output_dir_suffix) if len(chosen_candidates) > 0 else output_dir
        write_3d_binary_image(dicom_dir, output_path, chosen, consensus=consensus_strategy)
